public class AVL2 {
    ...
    private void updateBalance(Node n) {
    ...
    public Node DLR(Node x){
        x.left = leftRotate(x.left);
        return rightRotate(x);
    }

    public Node DRR(Node x){
    ...
    public Node successor(Node n){
        ...
        Node cur = this.root;
        while (cur != null) {
            boolean result = search(cur.left,n.key);
    ...
    public Node predecessor(Node n){
        ...
        Node cur = this.root;
        while (cur != null){
            boolean result = search(cur.right, n.key);
            if (result == true) return cur;
            else {
                cur = cur.left;
            }
